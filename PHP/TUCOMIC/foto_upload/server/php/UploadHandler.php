...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
		...
		//$path=str($id_item);
		$estructura =dirname($this->get_server_var('SCRIPT_FILENAME')).'/files/'.$id_item.'/';	
        if(!file_exists($estructura)){ //VE SI LA CARPETA EXISTE
	        mkdir($estructura, 0777);//CREAR CARPETA CN TODOS LOS PERMISOS
		}
			
        $this->options = array(
            'script_url' => $this->get_full_url().'/',
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/files/'.$id_item.'/',
            'upload_url' => $this->get_full_url().'/files/'.$id_item.'/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
		...
		session_start();
		$id_item=$_SESSION['id_item'];
		$conexion = pg_connect ("host=localhost port=5432 dbname=tucomic  user=postgres password=abcdef1234");
		$insertar = "insert into foto(id_item, nombre) values ($id_item,'$file->name');";
		$query=pg_query($insertar) or die("Error al registrar el nombre de la pelicula" . pg_last_error());
		// --- //
